...
# -*- coding: utf-8 -*-
"""
Video-MME evaluation – LongVA-7B-DPO (no delta, fp32)
→ 2700 条结果输出到 mme_video_longva_output.json
"""
...
# Frame cache（Tensor 形状 F×3×224×224）
CACHE_MAX = 24
...
for item in tqdm(dataset, total=len(dataset), desc="Processing"):
    ...
    if not pred:
        print(f"❓ no letter for {vid_id}: {decoded[:80]}")
