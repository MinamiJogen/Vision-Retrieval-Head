...
class LLMNeedleHaystackTester:
    ...
    def __init__(self,
        ...
        if "Qwen" in self.model_version:
            ...
            print(f"[DEBUG] Model loaded: {self.model_to_test.__class__.__name__}")
            print(f"[DEBUG] Model config _attn_implementation: {getattr(self.model_to_test.config, '_attn_implementation', 'N/A')}")
    ...
    def decode(self, q_outputs, inp, decode_len, block_list=None):
        ...
        for step_i in range(decode_len):
            inp = inp.view(1, 1)
            print(f"[DEBUG] Step {step_i}: Decoding token {inp.item()}, feeding into model with torch attention")
            outputs = self.model_to_test(
                input_ids=inp, 
                past_key_values=past_kv, 
                use_cache=True, 
                output_attentions=True, 
                attn_mode="torch"  # ✅ 这里强制 torch
            )
            print(f"[DEBUG] Model output keys: {outputs.keys()}")
            if outputs.attentions is not None:
                print(f"[DEBUG] Attention shape (layer 0): {outputs.attentions[0].shape}")
            else:
                print("[DEBUG] No attention returned! Possible flash attention path?")
            ...
            if step_token == '<0x0A>' or inp.item() == 144:
                print("[DEBUG] Reached stopping condition")
                break
    ...
    def evaluate_and_log(self, context_length, depth_percent):
        ...
        with torch.no_grad():
            print("[DEBUG] Running initial forward pass (likely flash attention)")
            q_outputs = self.model_to_test(input_ids=input_ids[:,:-1], use_cache=True, return_dict=True)
            print("[DEBUG] Initial output keys:", q_outputs.keys())
