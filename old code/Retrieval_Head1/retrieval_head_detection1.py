...
def hook_attention(module, input, output, layer_idx):
    # 如果输出是 tuple 且包含注意力张量（通常在 output[1] 中），则保存
...
def print_decoded_tokens(input_ids, tokenizer):
    ...
    for idx, token in enumerate(tokens):
        print(f"{idx}: {token}")
    if IMAGE_TOKEN_INDEX in input_ids.squeeze(0).tolist():
        pos = input_ids.squeeze(0).tolist().index(IMAGE_TOKEN_INDEX)
        print(f"<image> token found at position: {pos}")
    else:
...
#     device_map = f"cuda:{selected_gpu}"
#     device = torch.device(device_map)
# else:
#     device_map = "cpu"
#     device = torch.device("cpu")
# print(f"自动选择的 GPU: {device_map}")
# if device.type == "cuda":
#     torch.cuda.set_device(selected_gpu)
#     print("PyTorch 当前默认设备：", torch.cuda.current_device())
...
class LLMNeedleHaystackTester:
    def __init__(self,
        ...
        if document_depth_percents is None:
            if document_depth_percent_interval_type == 'linear':
                self.document_depth_percents = np.round(np.linspace(document_depth_percent_min,
                                                                    document_depth_percent_max,
                                                                    num=document_depth_percent_intervals,
                                                                    endpoint=True)).astype(int)
            elif document_depth_percent_interval_type == 'sigmoid':
                self.document_depth_percents = [self.logistic(x) for x in np.linspace(document_depth_percent_min,
                                                                                       document_depth_percent_max,
    ...
    def run_test(self, args):
        pairs = [(cl, dp) for cl in self.context_lengths if cl >= args.s_len and cl <= args.e_len
                 for dp in self.document_depth_percents]
        for context_length, depth_percent in tqdm(pairs, desc="Testing", unit="sample"):
            self.bound_evaluate_and_log(context_length, depth_percent)
    
    def retrieval_calculate(self, attention_matrix, retrieval_score, inp, step_token, topk=1):
    ...
    def decode(self, q_outputs, inp, decode_len, block_list=None):
        ...
        for step_i in range(decode_len):
            ...
            outputs = self.model_to_test(input_ids=inp, past_key_values=past_kv, use_cache=True,
                                          output_attentions=True, attn_mode="torch")
    ...
    def evaluate_and_log(self, context_length, depth_percent):
        ...
        if self.save_results:
            ...
            with open(p, 'w') as f:
                json.dump(results, f)
        del input_ids, q_outputs, output, retrieval_score, context, question, response
        gc.collect()
...
if __name__ == "__main__":
    ...
    parser.add_argument('-s', '--s_len', metavar='N', type=int, help='起始 context 长度')
    parser.add_argument('-e', '--e_len', metavar='N', type=int, help='结束 context 长度')
    parser.add_argument('--model_path', type=str, default=None, help='模型路径')
    parser.add_argument('--model_name', type=str, default=None, help='模型名称')
    parser.add_argument('--model_name_suffix', type=str, default=None, help='模型后缀')
    parser.add_argument('--model_provider', type=str, default="LLaMA", help='使用哪个模型')
