...
def analyze_input_attention_heads(attn_maps, input_token_length, split_index):
    ...
    for layer in sorted_layers:
        ...
        # 截取 input token 部分（注意：这里使用的 token 索引包含所有 token，后续会忽略第一个 token）
        ...
        # 计算每个 head 对视觉 token 的注意力比例
        # 排除第一个 token：对 attn_sub 的 key 维度，取索引从 1 开始；
        # 此时视觉 token 在新序列中的起始索引为 (split_index - 1)
        for h in range(num_heads):
            # 求和时沿着 query 维度（即 axis=0），得到每个 key token 被所有 query 关注的总和
            token_sum_h = attn_sub[h].sum(axis=0)
            total_attn = token_sum_h.sum()
            # 视觉部分：从 (split_index - 1) 开始的 token 的总关注度
            visual = np.sum(token_sum_h[split_index - 1:]) if total_attn > 0 else 0.0
            ratio = visual / total_attn if total_attn > 0 else 0.0
            head_image_ratios[layer, h] = ratio

        # 计算所有 head 平均的 token 级注意力分布（归一化后乘以 1e3）
        # 这里先对所有 head 求均值，得到 shape (query_length, key_length)，再对 key 维度求和
        mean_attn = np.mean(attn_sub, axis=0)  # (query_length, key_length)
        token_sum = mean_attn.sum(axis=0)        # 对 query 维度求和，得到每个 key token 的累计注意力
        total = token_sum.sum()
        if total != 0:
        ...
        # 计算每个 head 对各 token 的注意力分布（归一化后乘以 1e3）
        for h in range(num_heads):
            token_sum_h = attn_sub[h].sum(axis=0)  # 沿着 query 维度求和
