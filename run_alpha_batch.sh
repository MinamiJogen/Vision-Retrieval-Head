...
FULL_LOG="./mergelogs/full_output_${TIMESTAMP}.log"
...
for a in "${alphas[@]}"; do
    ...
    # 目录 / 文件名
    outdir="/disk3/minami/huggingface/hub/models--LongVA-Merge-a${a}"
    logfile="./mergelogs/alpha${a}.json"
    suffix="videomme_longva_a${a}"
    
    # 为每个 alpha 创建单独的日志文件（可选）
    individual_log="./mergelogs/alpha${a}_${TIMESTAMP}.log"

    # 1) 合并模型
    echo ">>> 开始合并模型..."
    python merge-givin.py --alpha "${a}" --output_dir "${outdir}" 2>&1 | tee -a "${individual_log}"
